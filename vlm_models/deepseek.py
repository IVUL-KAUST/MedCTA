...
# compatibility shim for torch 2.0.1
...
def build_conversation(question: str, image_path: str):
    return [
        {
            "role": "<|User|>",
            "content": f"<image>\n{SYSTEM_PROMPT}\nQuestion: {question}\nAnswer:",
...
def run_inference(model, processor, item: Dict[str, Any]) -> str:
    conversation = build_conversation(item["question"], item["resolved_image"])
    ...
    prepare_inputs = move_to_device(prepare_inputs, DEVICE)
...
def main() -> None:
    ...
    results = sorted(results, key=lambda x: int(x["id"]) if str(x["id"]).isdigit() else str(x["id"]))
    total = len(results)
    correct = sum(int(x["match"]) for x in results)
    accuracy = correct / total if total else 0.0
    avg_semantic_score = sum(float(x["semantic_score"]) for x in results) / total if total else 0.0
