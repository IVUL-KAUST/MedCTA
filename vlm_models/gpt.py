...
def build_input(question: str, image_path: str) -> List[Dict[str, Any]]:
...
def run_inference(client: OpenAI, item: Dict[str, Any]) -> str:
    response = client.responses.create(
        model=MODEL_NAME,
        input=build_input(item["question"], item["resolved_image"]),
        max_output_tokens=MAX_OUTPUT_TOKENS,
    )
    pred = getattr(response, "output_text", "")
    return pred.strip()
    ...
    for item in data:
        ...
        if not os.path.exists(resolved_image):
            ...
            try:
                ...
                semantic_score = evaluate_prediction(client, pred, item["answer"])
