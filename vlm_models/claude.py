...
def run_inference(client: Anthropic, item: Dict[str, Any]) -> str:
    mime_type = guess_mime_type(item["resolved_image"])
    image_b64 = file_to_base64(item["resolved_image"])
...
def main() -> None:
    if not ANTHROPIC_API_KEY:
        raise ValueError("ANTHROPIC_API_KEY is not set. Please export it before running.")
    ...
    for item in data:
        ...
        if not os.path.exists(resolved_image):
            ...
            try:
                ...
                pred = run_inference(anthropic_client, eval_item)
